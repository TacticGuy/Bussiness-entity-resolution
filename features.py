...
def compute_features(name1, addr1, name2, addr2, country1="", country2=""):
    ...
    Returns a dict with:
      ...
      - postal_exact_match: bool, True iff both addresses yield a postal code
        and the codes match exactly
            - country_exact_match: bool, True when both country labels match
    """
    ...
    name1_norm, name2_norm = normalize_name(name1), normalize_name(name2)
    addr1_norm, addr2_norm = normalize_address(addr1), normalize_address(addr2)
    name_token_sort = fuzz.token_sort_ratio(name1_norm, name2_norm)
    addr_token_sort = fuzz.token_sort_ratio(addr1_norm, addr2_norm)
    name_jaro_winkler = JaroWinkler.similarity(name1_norm, name2_norm) * 100.0
 
    pc1 = _extract_postal_code(addr1_norm)
    pc2 = _extract_postal_code(addr2_norm)
    ...
    return {
        ...
        "country_exact_match": bool(country1 and country2 and
                         str(country1).casefold().strip() == str(country2).casefold().strip()),
    }
...
def build_training_table(candidate_pairs_path, ground_truth_path,
    ...
    for _, row in pairs.iterrows():
        ...
        feats = compute_features(name1, addr1, name2, addr2,
                     lookup.at[s1_id, "country"], lookup.at[c_id, "country"])
    ...
    cols.insert(-1, "country_exact_match")
